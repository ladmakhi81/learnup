...
func main() {
	...
	userSvc := userService.NewUserSvcImpl(unitOfWork, i18nTranslatorSvc)
	notificationSvc := notificationService.NewNotificationServiceImpl(unitOfWork, i18nTranslatorSvc)
	validationSvc := validatorv10.NewValidatorSvc(validator.New(), i18nTranslatorSvc)
	authSvc := authService.NewAuthServiceImpl(redisSvc, tokenSvc, i18nTranslatorSvc, unitOfWork)
	categorySvc := categoryService.NewCategoryServiceImpl(unitOfWork, i18nTranslatorSvc)
	courseSvc := courseService.NewCourseServiceImpl(unitOfWork, i18nTranslatorSvc)
	ffmpegSvc := ffmpegv1.NewFfmpegSvc()
	videoSvc := videoService.NewVideoServiceImpl(unitOfWork, minioSvc, ffmpegSvc, logrusSvc, i18nTranslatorSvc)
	...
	teacherCourseSvc := teacherService.NewTeacherCourseServiceImpl(unitOfWork, i18nTranslatorSvc)
	teacherVideoSvc := teacherService.NewTeacherVideoServiceImpl(unitOfWork, i18nTranslatorSvc)
	teacherCommentSvc := teacherService.NewTeacherCommentServiceImpl(unitOfWork, i18nTranslatorSvc)
	commentSvc := commentService.NewCommentServiceImpl(unitOfWork, i18nTranslatorSvc)
	likeSvc := likeService.NewLikeServiceImpl(unitOfWork, i18nTranslatorSvc)
	cartSvc := cartService.NewCartService(unitOfWork, i18nTranslatorSvc)
	questionSvc := questionService.NewQuestionServiceImpl(unitOfWork, i18nTranslatorSvc)
	questionAnswerSvc := questionService.NewQuestionAnswerServiceImpl(unitOfWork, i18nTranslatorSvc)
	teacherQuestionSvc := teacherService.NewTeacherQuestionServiceImpl(unitOfWork, i18nTranslatorSvc)
	...
	transactionSvc := transactionService.NewTransactionService(unitOfWork)
	paymentSvc := paymentService.NewPaymentService(unitOfWork, zarinpalSvc, zibalSvc, stripeSvc, config, i18nTranslatorSvc)
	orderSvc := orderService.NewOrderService(unitOfWork, i18nTranslatorSvc, paymentSvc)
